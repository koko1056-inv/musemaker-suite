...
export function ShareConversationDialog({
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("share-conversation-content", {
        body: {
          conversationId,
          shareType: activeTab,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>会話内容を共有</DialogTitle>
        </DialogHeader>

        <div className="text-sm text-muted-foreground mb-4">
          <span className="font-mono bg-muted px-2 py-0.5 rounded">{phoneNumber}</span> との会話を共有
        </div>
