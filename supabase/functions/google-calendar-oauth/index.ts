...
serve(async (req) => {
  ...
  try {
    ...
    if (url.searchParams.has("code")) {
      ...
      return new Response(
        `<html><body><script>window.opener?.postMessage({ type: 'google-calendar-oauth-success', integration_id: '${integration_id}', csrf_nonce: '${csrfNonce}' }, '*'); setTimeout(() => window.close(), 1000);</script><h2>Google Calendar認証が完了しました。このウィンドウは自動的に閉じます。</h2></body></html>`,
        { headers: { ...corsHeaders, "Content-Type": "text/html" } }
      );
    }
