...
serve(async (req) => {
  ...
  try {
    ...
    const { conversationId, shareType, webhookUrl, recipientEmail, senderName }: ShareRequest = await req.json();
    ...
    const agentName = conversation.agent?.name || "エージェント";

    // Get extracted data
    const { data: extractedDataRows } = await supabase
      .from("conversation_extracted_data")
      .select("field_key, field_value")
      .eq("conversation_id", conversationId);

    const extractedData = extractedDataRows || [];
    ...
    // Format date
    const callDate = new Date(conversation.started_at);
    ...
    if (shareType === "slack") {
      ...
      let htmlContent = `
        ...
        <head>
          ...
          <style>
            ...
            .summary { border-left: 3px solid #3b82f6; padding-left: 12px; }
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin: 0; font-size: 20px;">📞 受電記録が共有されました</h1>
              <p style="margin: 8px 0 0; opacity: 0.9;">エージェント: ${agentName}</p>
            </div>
            <div class="content">
              <div class="info-grid">
                <div class="info-item">
                  <div class="info-label">発信者</div>
                  <div class="info-value">${conversation.phone_number || "不明"}</div>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                  ...
                  <div class="info-value">${conversation.status}</div>
                </div>
              </div>
      `;
      ...
      if (transcriptArray && transcriptArray.length > 0) {
        const transcriptHtml = transcriptArray.map((msg) => {
          ...
          const roleLabel = msg.role === "agent" ? "🤖 AI" : "👤 お客様";
      ...
      const emailResponse = await resend.emails.send({
        ...
        to: [recipientEmail],
        subject: `[共有] 受電記録 - ${agentName} (${conversation.phone_number || "不明"})`,
        html: htmlContent,
      });
    ...
    console.error("Error in share-conversation-content function:", error);
    return new Response(
